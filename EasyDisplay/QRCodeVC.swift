...
//  EasyDisplay
//
//  Created by Mohammed Tillawy on 7/30/18.
...
class QRCodeVC: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    var callbackConnection = { (con: Connection) -> () in
    ...
    func found(code: String) {
        ...
        guard let data = code.data(using: .ascii, allowLossyConversion: true) else { return }
        let conn = try! decoder.decode(Connection.self, from: data)
